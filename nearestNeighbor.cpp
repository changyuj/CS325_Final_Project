...
std::vector<std::pair<int, int>> nearestNeighbor(std::vector<std::pair<int, std::pair<int, int>>>& cityList, std::unordered_map<int, 
	...
	std::ofstream outputFile("tspLists.txt");
	...
	for (int i = 0; i < cityList.size(); i++) {
		...
		outputFile << "\nVertex " << cityList[i].first << "\n";
		for (int j = 0; j < adjacencyListWithWeights[cityList[i].first].size(); j++) {

			outputFile << adjacencyListWithWeights[cityList[i].first][j].second << std::endl;
		}
		outputFile << "\n";
	}
